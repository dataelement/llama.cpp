...
function run_qwen_32b() {
    ...
    ./build/bin/server -m $model -c 30000 -n 2024 --host 0.0.0.0 --port 9301 --n-gpu-layers 100 -cb -ub 32 -np 8
}
...
function run_qwen_72b() {
    ...
    ./build/bin/server -m $model -c 30000 -n 2024 --host 0.0.0.0 --port 9300 --n-gpu-layers 100 -cb -ub 32 -np 8
}

function run_cohere() {
    export CUDA_VISIBLE_DEVICES=2
    model="/opt/bisheng-rt/models/model_repository/command-r-gguf/c4ai-command-r-v01-Q5_K_M.gguf"
   ./build/bin/server -m $model -c 60000 -n 2048 --host 0.0.0.0 --port 9100 --n-gpu-layers 100 --chat-template command-r -cb -ub 32 -np 8
}
...
function run_command_r_plus() {
   ...
   ./build/bin/server -m $model -c 100000 -n 2048 --host 0.0.0.0 --port 9100 --n-gpu-layers 200 --chat-template command-r -cb -ub 32 -np 8
}
