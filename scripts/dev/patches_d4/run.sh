...
function prepare() {
    ...
    # docker run -itd ${PROXY_ENV} --gpus all --net=host --name llamacpp-dev-cu122 -v $HOME:$HOME -v /mnt/disks:/mnt/disks $BASE_CUDA_DEV_CONTAINER bash
    # docker exec llamacpp-dev-cu122 apt update
    # docker exec llamacpp-dev-cu122 apt install -y cmake ccache

    PROXY_ENV="-e HTTP_PROXY= -e HTTPS_PROXY -e http_proxy= -e https_proxy="
    docker run -itd ${PROXY_ENV} --gpus all --net=host --name llamacpp-dev-cu122 -v $HOME:$HOME -v /mnt/disks:/mnt/disks llamacpp_build:0.1 bash
